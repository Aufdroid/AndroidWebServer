...
public class AndroidWebServer extends NanoHTTPD {
    ...
    private boolean mIsDeutscheBahn = true;

    private double mLatGermany = 49.29154586791992;
    private double mLonGermany = 11.403055191040039;
    /*
    */
    private double mLatItaly = 42.495;
    private double mLonItaly = 12.311;
    ...
    public Response serve(IHTTPSession session) {
        Log.d("com.swissqual.qp.UrlGps", "serving... " + (mIsDeutscheBahn ? "Germany" : "Italy"));
        if (mIsDeutscheBahn) {
            response = getMsgDeutschland(System.currentTimeMillis());
        } else {
            response = getItaly(System.currentTimeMillis());
        }
    ...
    public void setTrainType(boolean isDeutscheBahn) {
        mIsDeutscheBahn = isDeutscheBahn;
        if (isDeutscheBahn) {
            Log.d("com.swissqual.qp.UrlGps", "Germany");
        } else {
            Log.d("com.swissqual.qp.UrlGps", "Italy");
        }
    }


    private double roughDegreesPer1m = 0.00009; // 360 / 40 Mm
    int secondsPerHalfLoop = 50;
    double metersPerSecond = 10;

    private double getLat(long epochTimeSec, double baseLattitude) {
        double distance = getDistance(epochTimeSec);
        double angleRad = getAngleRad(epochTimeSec);
        double lat = baseLattitude + distance * Math.sin(angleRad);
        Log.d("com.swissqual.qp.UrlGps", String.format("lat calculation: %s + %s m with angle %s rad = %s", baseLattitude, distance, angleRad, lat));
        return lat;
    }

    private double getLon(long epochTimeSec, double baseLongitude) {
        double distance = getDistance(epochTimeSec);
        double angleRad = getAngleRad(epochTimeSec);
        double lon = baseLongitude + distance * Math.cos(angleRad);
        Log.d("com.swissqual.qp.UrlGps", String.format("lon calculation: %s + %s m with angle %s rad = %s", baseLongitude, distance, angleRad, lon));
        return lon;
    }

    private double getAngleRad(long epochTimeSec) {
        double hourPart = (epochTimeSec / 3600.0) % 1; // value [0...1)
        return hourPart * 2 * Math.PI;
    }

    private double getDistance(long epochTimeSec) {
        int secPart = Math.abs(secondsPerHalfLoop - (int) (epochTimeSec % (2 * secondsPerHalfLoop)));
        return secPart * roughDegreesPer1m * metersPerSecond;
    }

    private int getSpeed(long epochTimeSec) {
        return 70 + (int) (epochTimeSec % 60);
    }

    private String response;

    private String getMsgDeutschland(long epochTimeMs) {
        long epochTimeSec = epochTimeMs / 1000;
        String speed = String.format("%s", (int) getSpeed(epochTimeSec));
        String lat = String.format("%s", getLat(epochTimeSec, mLatGermany));
        String lon = String.format("%s", getLon(epochTimeSec, mLonGermany));
        String serverTime = String.format("%s", epochTimeMs);
        String time = new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss").format(new Date(epochTimeMs));

        return String.format("{\"connection\":true,\"servicelevel\":\"AVAILABLE_SERVICE\",\"speed\":%s,\"gpsStatus\":\"VALID\",\"latitude\":%s,\"longitude\":%s,\"serverTime\":%s,\"wagonClass\":\"SECOND\",\"navigationChange\":\"%s\"}",
                speed,
                lat,
                lon,
                serverTime, // e. g. 1517990410340 in ms, epoch time UTC
                time); // e. g. 2018-02-07-04-17-40 UTC?
    }
    ...
    private String getItaly(long epochTimeMs) {
        long epochTimeSec = epochTimeMs / 1000;
        String speed_kmPerHour = String.format("%s", (int) getSpeed(epochTimeSec));
        String latWith3Decimals = String.format("%.3f", getLat(epochTimeSec, mLatItaly));
        String lonWith3Decimals = String.format("%.3f", getLon(epochTimeSec, mLonItaly));

        Log.d("com.swissqual.qp.UrlGps", String.format("Lat '%s', Lon '%s', Speed '%s'", latWith3Decimals, lonWith3Decimals, speed_kmPerHour));
        return getItaly(latWith3Decimals, lonWith3Decimals, speed_kmPerHour);
    }

    private String getItaly(String lat, String lon, String speed) {
        return "\n" +
                "<!DOCTYPE>\n" +
                "<html>\n" +
                "\n" +
                ...
                String.format("\t\t\t<span id=\"ivSpeedValue\" class=\"orangeGlow\">%s</span>&nbsp;<span id=\"ivSpeedUnit\">Km/h</speed>\n", speed) + // %s = 111 km/h
                "\t\t</div>\n" +
                "\t</div>\n" +
                "\t<div id=\"ivGpsData\">\n" +
                ...
                "\t\t\t\t<tr>\n" +
                "\t\t\t\t\t<td>Lat.:</td>\n" +
                String.format("\t\t\t\t\t<td id=\"ivLat\">%s</td>\n", lat) + // %s = 42.495
                "\t\t\t\t\t<td>Lon.:</td>\n" +
                String.format("\t\t\t\t\t<td id=\"ivLon\">%s</td>\n", lon) + // %s = 12.311
                "\t\t\t\t</tr>\n" +
                "\t\t\t\t<tr>\n" +
                "\t\t\t\t\t<td>Compass:</td>\n" +
                "\t\t\t\t\t<td id=\"ivDir\">N-W</td>\n" +
